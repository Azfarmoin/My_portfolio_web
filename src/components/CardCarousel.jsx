...
function useIsSmallScreen() {
  ...
  useEffect(() => {
    ...
    // init + subscribe (Safari fallback)
    onChange();
...
const SingleCard = ({ project, compact = false }) => (
  <div
    className={`group flex flex-col rounded-3xl overflow-hidden shadow-xl bg-white/5 backdrop-blur-xl border border-white/10 
    ${
      compact
        ? "mx-8 my-3 max-w-xs"
        : "w-full max-w-[92vw] mx-auto my-3"
    }`}
  >
    <div
      className={`relative overflow-hidden ${
        compact ? "aspect-[16/9] max-h-28" : "aspect-[16/10]"
      }`}
    >
      <img
        src={project.image}
    ...
    <div
      className={`flex flex-col justify-between ${
        compact ? "p-4 min-h-[140px]" : "p-5 min-h-[180px]"
      }`}
    >
      <div>
        <h3
        ...
        <p
          className={`text-gray-300 leading-relaxed line-clamp-3 ${
          ...
          {allGroups.flatMap((group) =>
            group.projects.map((project, i) => (
              ...
              >
                ...
                <p className="text-teal-400 text-xs font-semibold uppercase tracking-widest mb-2 px-1 text-left">
                  {group.label}
                </p>
